...
B<NPEM_BACKEND> - Select the NPEM backend. Accepts I<auto> (the default),
I<kernel> and I<pci>. With I<auto>, ledmon probes each controller for the NPEM
kernel driver (CONFIG_PCI_NPEM) and uses it when present, otherwise it falls
back to accessing PCI config space directly. Setting I<kernel> forces use of the
kernel driver and I<pci> forces PCI config space access. The kernel driver is
preferred because it also works with _DSM LED control and it is not vulnerable
to PCI config space access lockdown.
