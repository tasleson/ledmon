...
B<USE_NPEM_DRIVER> - Use the NPEM kernel driver (CONFIG_PCI_NPEM).
This should be disabled for kernels that don't include the kernel NPEM driver.
The default value is true. If disabled, ledmon will try to access PCI config space.
Kernel driver is preferred because it also works with _DSM LED control and
it is not vulnerable on PCI config space access lockdown.
